...
var addFeed, currentFeedURL, feeds, generateFeed, generateFolder, generateOverview, getJsonFeed, hideDetail, init, refreshFeed, showAdd, showContent, showDetail, showMenu, toggle, toggleMenu;
...
showAdd = function() {
  ...
  return $("#quick-add-bubble-holder").toggleClass("hidden");
};

hideDetail = function(obj, item) {
  obj.removeClass("expanded");
  obj.find(".entry-container").remove();
  obj.find(".entry-actions").remove();
  return obj.click(function() {
    return showDetail(obj, item);
  });
};
...
addFeed = function() {
  var k;
  k = $("#quickadd").val();
  return getJsonFeed(k, function(feed) {
    var li;
...
generateFeed = function(url, title) {
  var li;
  ...
  li.find("a:first").click(function() {
    return showContent(url);
  });
  return li;
};

init = function() {
  var feed_ul, item, _i, _len, _results;
  feed_ul = $("#sub-tree-item-0-main ul:first");
  _results = [];
  for (_i = 0, _len = feeds.length; _i < _len; _i++) {
    item = feeds[_i];
    if (item.type === "rss") {
      _results.push(feed_ul.append(generateFeed(item.xmlUrl, item.title)));
    } else {
      _results.push(feed_ul.append(generateFolder(item)));
    }
  }
  return _results;
};
...
refreshFeed = function() {
  return getJsonFeed(currentFeedURL, function(feed) {
    localStorage.setItem(currentFeedURL, JSON.stringify(feed));
    return showContent(currentFeedURL);
  });
};

document.addEventListener('DOMContentLoaded', function() {
  $("#lhn-add-subscription").click(function() {
    return showAdd();
  });
